...
object Day3 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val solution = wires.map {
      case ((a, b), _) => Math.abs(a) + Math.abs(b)
      case _ => 0
  ...
  def process(wire: String): Map[(Int, Int), Int] = {
    wire.split(",").map(parseInstruction).foldLeft((0, 0), Map[(Int, Int), Int]()) {
      (acc, i) => {
        ...
        val map = scala.collection.mutable.Map[(Int, Int), Int]() ++ acc._2
  ...
  def combine(a: Map[(Int, Int), Int], b: Map[(Int, Int), Int]): Map[(Int, Int), Int] = {
    a.collect { case (k, v) if b.contains(k) => (k, v + b(k)) }
  }
